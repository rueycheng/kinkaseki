...
int main(int argc, char** argv) {
    ...
    if (!g["train"] && !g["test"]) bye("Specify either --train or --test");
    if (g["train"] && g["test"]) bye("Cannot run in both mode.  Specify either one");
	...
	// Save collection
	{
	    cerr << "Save collection" << "\n";
	    fs::ofstream collection_out(basedir / "collection");

	    collection_out << "@document " << documents.size() << "\n";
	    foreach (const posting& dp, documents) { collection_out << dp << "\n"; }
	}
